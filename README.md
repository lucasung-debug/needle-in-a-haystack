...
1. **FRAME via a staged dialogue** — instead of accepting the request at face value, it runs a *framing dialogue* (constructive Cartesian doubt + Socratic questioning, adapted from the `doubt` planning skill): surfaces hidden assumptions, strips the request to the real question (no premature topic/method), and **declares the governing research paradigm** — positivist, interpretivist, pragmatist, critical-realist, … — because the paradigm decides *what even counts as evidence*. It also fixes the *negative condition* (what would show there's no answerable needle).
