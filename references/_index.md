# references/_index.md — what each reference contains (then load exactly one)

> SKILL.md's routing table already says **when** to load each file. This catalog says **what's inside** each one —
> read it only when you're unsure which reference a phase needs, then open the **one** that matches. `research.md` is always first.

| File | What it is |
|------|------------|
| `research.md` | **The system directive (정전).** Runs the full cycle (FRAME→…→REPORT) and carries LAW 0, the 4 layers, the 6 axes, BYOK/cost, modes, and the compliance block. |
| `philosophy.md` | **The single root — Peirce's Abduction Loop** and the epistemic commitments everything derives from. |
...
| `falsify-skeptic.md` | **The skeptic-judge for FALSIFY** — author ≠ reviewer adversarial review over 7 named failure modes (confirmation-only, single-source, over-association, source-bias, stale, sycophancy, scope) + a PASS/REWORK scorecard; what an honest `[FALSIFY]=PASS` rests on. |
