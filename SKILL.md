...
6. At FALSIFY, run `references/falsify-skeptic.md` (author ≠ reviewer) to attack the leading candidate across seven named failure modes; fill `templates/skeptic-scorecard.snippet.md` and record its verdict as `[SKEPTIC]` in the compliance block. Mark `[FALSIFY]=PASS` **only** on a PASS scorecard — otherwise loop back (REWORK the named mode) or output **NEEDLE NOT FOUND**. The gate requires `[SKEPTIC]=PASS` to ship a needle.
